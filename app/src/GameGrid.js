...
export const GameGrid = ({ pawn1Pos, pawn2Pos, pawn1Moves, pawn2Moves, walls }) => {
    ...
    for (let i = 0; i < gridSize; i++) {
      ...
      for (let j = 0; j < gridSize; j++) {
        ...
        if (isXWall && !isYWall) {
          ...
          row.push(<GridCell pos={`${i}-${j}`} classes={`w-[${cellSize}px] h-[${cellSize}px] ${cellColor}`} onClick={() => {}}></GridCell>)
        }
      }
  ...
  grid[pawn1R].props.children[pawn1C] = <GridCell pos={`${pawn1R}-${pawn1C}`} classes={`w-[${cellSize}px] h-${cellSize}px] ${cellColor}`} children={<Pawn pawnColor={`${p1Color}`} onClick={handlePawn1Click} pawnID={`${pawn1R}-${pawn1C}`} />} onClick={() => {}} />;
  grid[pawn2R].props.children[pawn2C] = <GridCell pos={`${pawn2R}-${pawn2C}`} classes={`w-[${cellSize}px] h-${cellSize}px] ${cellColor}`} children={<Pawn pawnColor={`${p2Color}`} onClick={handlePawn2Click} pawnID={`${pawn2R}-${pawn2C}`} />} onClick={() => {}} />;
  if (pawn1Clicked) {
    for (let i = 0; i < pawn1Moves.length; i++) {
      let r = pawn1Moves[i].split('-')[0];
      let c = pawn1Moves[i].split('-')[1];
      grid[r].props.children[c] = <GridCell pos={`${r}-${c}`} classes={`w-[${cellSize}px] h-${cellSize}px] ${cellColorHl}`} children={<div></div>} onClick={handle1CellsClick} />
    }
  }
  if (pawn2Clicked) {
    for (let i = 0; i < pawn2Moves.length; i++) {
      ...
      grid[r].props.children[c] = <GridCell pos={`${r}-${c}`} classes={`w-[${cellSize}px] h-${cellSize}px] ${cellColorHl}`} children={<div></div>} onClick={handle2CellsClick} />
    }
  }
  if (cells1Clicked) {
    for (let i = 0; i < pawn1Moves.length; i++) {
      let r = pawn1Moves[i].split('-')[0];
      let c = pawn1Moves[i].split('-')[1];
      grid[r].props.children[c] = <GridCell pos={`${r}-${c}`} classes={`w-[${cellSize}px] h-${cellSize}px] ${cellColor}`} children={<div></div>} onClick={() => {}} />
    }
  }
  if (cells2Clicked) {
    for (let i = 0; i < pawn2Moves.length; i++) {
